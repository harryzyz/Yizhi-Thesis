...
\begin{enumerate}[label=\alph*)]
  ...
  \item Otherwise, set $\mathfrak{h} = \mathfrak{h}/2$.  If
    \begin{enumerate}[label=(\roman*)]
      ...
      \item Otherwise, choose $$n_{k+1}=(n_k-1)\left\lceil\frac{1}{2(n_i-1)\mathfrak{h}}\right\rceil.$$
        (need double check)
        Go to Step 1.
    \end{enumerate}
\end{enumerate}
...
\begin{theorem}\label{uppbndcostSimp}
    Let $N(f,\varepsilon)$ denote the final number of $n_l$ in Step 3 when the algorithm terminates. Then this number is bounded below and above in terms of the true, yet unknown, $\Var(f')$.
