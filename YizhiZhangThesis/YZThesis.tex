...
\end{equation}
for some $x_j\in (u_{j-1},u_{j})$. Let $\{a=x_{0}, x_{1},\cdots,x_{n},x_{n+1}=b\}$ be a partition as was introduced just below \eqref{defvhat}. Note that no matter how the $x_i$'s are located, the largest possible width cannot be larger than two intervals. So $\text{size}(\{x_i\}_{i=0}^{n+1})\leq 2h=2(b-a)/n$.
...
Therefore by combining \eqref{errorbound}, \eqref{defcone}, and \eqref{vtilde1} together, we will obtain the error bound for our trapezoidal rule algorithm using only function values:
\begin{align*}%\label{errortrapcone}
  \overline{\text{err}}(f,n):=&C(n)\Var(f^{(p)}),\\
  \leq & C(n)\mathfrak{C}(\text{size}(\{x_i\}_{i=0}^{n+1}))\widehat{V}(f^{(p)},\{x_i\}_{i=0}^{n+1}),\\
  =& C(n)\mathfrak{C}(\text{size}(\{x_i\}_{i=0}^{n+1}))\widetilde{V}_1(f,n),\\
  =& \frac{(b-a)^2\mathfrak{C}(2(b-a)/n)\widetilde{V}_1(f,n)}{8n}
\end{align*}
(fix equation number)
