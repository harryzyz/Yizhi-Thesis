...
\end{equation}
For trapezoidal rule, $C(n)=(b-a)^2/8n^2, p=1$ (ref), and for Simpson's rule, $C(n)=(b-a)^4/5832n^4, p=3$ (ref). check 5832
...
\end{equation}
where $\{x_i\}_{i=0}^{n+1}$ is any given partition with not necessarily equal spacing, and $a=x_{0}\leq x_{1}<\cdots<x_{n}\leq x_{n+1}=b$.
The use of $n+2$ points and ignoring $x_0$ and $x_{n+1}$ in \eqref{defvhat} are for convenience later.
...
\end{equation}
To ensure a finite error bound, our algorithms are defined for function spaces with finite interval and finite total variations of the relative derivatives:
\begin{equation}\label{defspace}
 \cv^{p}:=\{f\in C[a,b]: \Var(f^{(p)}) < \infty \},
\end{equation}
where for trapezoidal rule, $p=1$, and for Simpson's rule, $p=3$.

Our algorithms will not work for all $f \in \cv^{p}$ because $f$ may have changes in $f^{(p)}$ on small intervals that the algorithms cannot detect. In order to build adaptive automatic and guaranteed algorithms, we have to set up the following pre-existing condition that all functions to be integrated must lie in a cone space of integrands for which $\widehat{V}(f,\{x_i\}_{i=0}^{n+1})$ does not underestimate $\Var(f)$ too much:
...
\end{multline}
The cone space $\cc^{p}$ is a subspace of $\cv^{p}$. Here $\text{size}(\{x_i\}_{i=0}^{n+1})$ is the largest possible width between any adjunct $x_i$:
...
We used the backward finite difference method to approximate $f'$. Let $h_1=u_{j+1}-u_{j}=(b-a)/n$ and
\begin{align*}
  f[u_{j}]&=f(u_{j}), \text{ for } j=0,\cdots, n,\\
  f[u_{j},u_{j-1}]&=\frac{f(u_{j})-f(u_{j-1})}{h_1},\text{ for } j=1, \cdots, n.
\end{align*}
...
\begin{equation}\label{vtileqfprime}
  f'(x_j)=\frac{f(u_{j})-f(u_{j-1})}{h_1}=\frac{n}{(b-a)}[f(u_{j})-f(u_{j-1})],
\end{equation}
for some $x_j\in (u_{j-1},u_{j})$. Let $\{a=x_{0}, x_{1},\cdots,x_{n},x_{n+1}=b\}$ be a partition as was introduced just below \eqref{defvhat}. Note that no matter how the $x_j$'s are located, the largest possible width cannot be larger than two intervals. So $\text{size}(\{x_j\}_{j=0}^{n+1})\leq 2h_1=2(b-a)/n$.
...
Therefore by combining \eqref{errorbound}, \eqref{defcone}, and \eqref{vtilde1} (put numbers to the equation below) together, we obtain the error bound for our trapezoidal rule algorithm using only function values:
\begin{align*}%\label{errortrapcone}
  \overline{\text{err}}(f,n):=&C(n)\Var(f'),\\
  \leq & C(n)\mathfrak{C}(\text{size}(\{x_i\}_{i=0}^{n+1}))\widehat{V}(f',\{x_i\}_{i=0}^{n+1}),\\
  =& C(n)\mathfrak{C}(\text{size}(\{x_i\}_{i=0}^{n+1}))\widetilde{V}_1(f,n),\\
  =& \frac{(b-a)^2\mathfrak{C}(2(b-a)/n)\widetilde{V}_1(f,n)}{8n^2}
\end{align*}
(fix equation number, explain C is non-decreasing, double check $n^2$ at bottom)

This error bound of our algorithm  using only function values provides guarantees that it will succeed. With a user provided tolerance $\varepsilon$, as long as $n$ is big enough, the approximation error will eventually decrease below $\varepsilon$.

\Section{Simpson's Rule}

Similar to the trapezoidal rule, we used the third order backward finite difference methods to approximate $f'''$. Let $h_3=v_{j+1}-v_{j}=(b-a)/6n$ and
\begin{align*}
  f[v_{j}]&=f(v_{j}), \text{ for } j=0,\cdots, 6n,\\
  f[v_{j},v_{j-1}]&=\frac{f(v_{j})-f(v_{j-1})}{h_3},\text{ for } j=1, \cdots, 6n,\\
  f[v_{j},v_{j-1},v_{j-2}]&=\frac{f(v_{j})-2f(v_{j-1})+f(v_{j-2})}{2h_3^2},\text{ for } j=2, \cdots, 6n,\\
  f[v_{j},v_{j-1},v_{j-2},v_{j-3}]&=\frac{f(v_{j})-3f(v_{j-1})+3f(v_{j-2})-f(v_{j-3})}{6h_3^3}, \text{ for } j=3, \cdots, 6n.
\end{align*}
...
\begin{multline}\label{vtileqftriprime}
  f'''(x_j)=\frac{f(v_{3j})-3f(v_{3j-1})+3f(v_{3j-2})-f(v_{3j-3})}{h_3^3},\\=\frac{216n^3}{(b-a)^3}[f(v_{3j})-3f(v_{3j-1})+3f(v_{3j-2})-f(v_{3j-3})].
\end{multline}
for some $x_j\in (v_{j-1},v_{j})$. Let $\{a=x_{0}, x_{1},\cdots,x_{n},x_{n+1}=b\}$ be a partition as was introduced just below \eqref{defvhat}. Note that no matter how the $x_j$'s are located, the largest possible width cannot be larger than six intervals. So $\text{size}(\{x_j\}_{i=0}^{n+1})\leq 6h_3=(b-a)/n$.
...
Therefore by combining the relative equations (\eqref{errorbound}, \eqref{defcone}, and \eqref{vtilde1}) (put numbers to the equation below) together, we obtain the error bound for our Simpson's rule algorithm using only function values:
\begin{align*}%\label{errortrapcone}
  \overline{\text{err}}(f,n):=&C(n)\Var(f'''),\\
  \leq & C(n)\mathfrak{C}(\text{size}(\{x_i\}_{i=0}^{n+1}))\widehat{V}(f''',\{x_i\}_{i=0}^{n+1}),\\
  =& C(n)\mathfrak{C}(\text{size}(\{x_i\}_{i=0}^{n+1}))\widetilde{V}_3(f,n),\\
  =& \frac{(b-a)^4\mathfrak{C}((b-a)/n)\widetilde{V}_3(f,n)}{5832n^4}
\end{align*}
