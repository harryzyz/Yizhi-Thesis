...
Secondly, if \eqref{neccontrap} does not meet, the cut-off value $\mathfrak{h}$ is halved to make the cone function space more inclusive. The reason is that the inflation factor defined in \eqref{definflationfactor} is decreasing with respect to $\mathfrak{h}$. By halving $\mathfrak{h}$, the inflation factor becomes larger. Intuitively the cone space is more inclusive. Integrands have higher chances of lying in the new cone than the old one, thus have higher chancec to be accurately calculated.
...
Let $\mathfrak{h}\le (b-a)/6$. Set $k=0$, $n_{k}=0$. For any integrand $f$ and error tolerance $\varepsilon$, do the following: %$\eta_{0}=\infty$,
\begin{description}
...
\Section{Trapezoidal Rule Complexity}
...
\Section{Simpson's Rule Complexity}
