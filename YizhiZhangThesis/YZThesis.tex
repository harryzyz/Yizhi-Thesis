...
In the previous chapter, I introduced the univariate integration problem, $\text{INT}(f)=\int_{a}^{b}f(x)dx\in\reals$. (\textcolor[rgb]{1.00,0.00,0.00}{how to make the equality stay in one line.}) In order to build adaptive, automatic algorithms with guarantees to solve the problem, we firstly consider two straight-forward yet widely used fixed cost algorithms, trapezoidal rule and Simpson's rule.
...
\end{equation}
It uses $n+1$ function values where those $n+1$ node points are equally spaced between $[a,b]$. In the meanwhile, the composite Simpson's rule can be defined as:
...
v_j=a+\frac{j(b-a)}{n}, \qquad j=0, \ldots, 6n, \qquad n\in \naturals.
\end{equation}
...
Finding a proper way to approximate the first and third order derivatives plays an important role in constructing our algorithms, especially in finding the upper bound of approximation error. If we look back at the traditional non-adaptive trapezoidal rule and Simpson's rule, the upper bound of approximation error can be represented in terms of the total variation of a particular derivative of integrand. 
...
To better define the total variation, $\Var(f)$, firstly we introduce $\widehat{V}(f,\{x_i\}_{i=0}^{n+1})$ as an approximation to the total variation:
\begin{equation}\label{defvhat}
    \widehat{V}(f,\{x_i\}_{i=0}^{n+1})=\sum_{i=1}^{n-1}|f(x_{i+1})-f(x_{i})|,
\end{equation}
where $\{x_i\}_{i=0}^{n+1}$ is any given partition with no necessity of equal spacing, and $a=x_{0}\leq x_{1}<\cdots<x_{n}\leq x_{n+1}=b$.
(\textcolor[rgb]{1.00,0.00,0.00}{1. explain two side points. 2. how to make the inequality stay in one line.})

From the definition of $\widehat{V}(f,\{x_i\}_{i=0}^{n+1})$, the total variation can be defined as an upper of bound of $\widehat{V}(f,\{x_i\}_{i=0}^{n+1})$:
\begin{equation}\label{defvar}
  \Var(f) := \sup_{n \in \naturals, \{x_i\}_{i=0}^{n+1}} \left\{\widehat{V}(f,\{x_i\}_{i=0}^{n+1})\right\},
\end{equation}

For an algorithm with finite error bound, it is trivial that the input functions must be in a space of integrands such that it is continuous in $[a,b]$ with finite total variations of derivatives, to the proper order, of the functions:
\begin{equation}\label{defspace}
 \cv^{p}[a,b]:=\{f\in C[a,b]: \Var(f^{(p)}) < \infty \}.
\end{equation}
For trapezoidal rule, $p=1$. For Simpson's rule, $p=3$.

In order to build an adaptive automatic and guaranteed algorithm, we have to set up the following pre-existing condition that all functions to be approximated must lie in a cone space of integrands for which $\widehat{V}(f,\{x_i\}_{i=0}^{n+1})$ does not underestimate $\Var(f)$ too much:
...
We use divided differences to explain The relationship between \eqref{defvhat} and \eqref{vtilde}. Let $h=v_{j+1}-v_{j}=(b-a)/3n$ and
\begin{align*}
  f[v_{j}]&=f(v_{j}), \text{ for } j=0,\cdots, 3n,\\
  f[v_{j},v_{j-1}]&=\frac{f(v_{j})-f(v_{j-1})}{h},\text{ for } j=1, \cdots, 3n,\\
  f[v_{j},v_{j-1},v_{j-2}]&=\frac{f(v_{j})-2f(v_{j-1})+f(v_{j-2})}{2h^2},\text{ for } j=2, \cdots, 3n,\\
  f[v_{j},v_{j-1},v_{j-2},v_{j-3}]&=\frac{f(v_{j})-3f(v_{j-1})+3f(v_{j-2})-f(v_{j-3})}{6h^3}, \text{ for } j=3, \cdots, 3n.
\end{align*}
