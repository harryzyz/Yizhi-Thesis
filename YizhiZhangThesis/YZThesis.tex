...
\end{equation}
The reason why we use $3n$ intervals with an even number of $n$ is for the convenience of notations of later deduction(???).

The error bound of trapezoidal rule and Simpson's rule can be presented in terms of the variation of the input fuction:

<<<<<<< HEAD
\begin{equation}\label{errorbound}
    \text{err}(f,n)\le\overline{\text{err}}(f,n):=h(n)\Var(f^{(p)}).
\end{equation}
For trapezoidal rule, $h_{t}(n)=(b-a)^2/8n^2, p=1$(ref). For Simpson's rule, $h_{s}(n)=(b-a)^4/5832n^4, p=3$.

The variation can be defined as follow:
\begin{equation}\label{defvar}
  \Var(f) := \sup_{\substack{n \in \naturals\\ a = x_0 < x_1 < \cdots < x_{n+1} =b}} \sum_{i=0}^{n} \abs{f(x_i+1)-f(x_{i})},
=======
...
>>>>>>> 21cebda6e7817fb755d3478589754a399e341b25
\end{equation}
where the input functions, in the space $\cf$, are continuos in $[a,b]$, and have finite variation to the order of 1 and 3, respectively, for trapezoidal rule and Simpson's rule:
\begin{equation}\label{defspace}
 \cf: =\cv^{k}[a,b]=\{f\in C[a,b]: \Var(f^{(k)}) < \infty \}, k=1,3.
\end{equation}
%
...
Given any partition $\{x_i\}_{i=0}^{n+1}$, where $a=x_{0}\le x_{1}\le\cdots\le x_{n}\le x_{n+1}=b$, we introduce an approximation to the variation as:
\begin{equation}\label{defvhat}
  \widehat{V}(f,\{x_i\}_{i=0}^{n+1})=\sum_{i=1}^{n-1}|f(x_{i+1})-f(x_{i})|
\end{equation}

By definition, $\widehat{V}(f,\{x_i\}_{i=0}^{n+1})$ is actually a lower bound of variation:
\begin{equation*}
  \widehat{V}(f,\{x_i\}_{i=0}^{n+1})\leq\sup_{\{x_i\}_{i=0}^{n+1}, n\in\naturals}\left\{\widehat{V}(f,\{x_i\}_{i=0}^{n+1})\right\}=\Var(f,n).
\end{equation*}

%For trapezoidale rule, we use $\widehat{V}_{1}(f',\{v_j\}_{j=0}^{n}$ to approximate $\Var(f')$, where %)=\norm[1]{f'-f(b)+f(a)}
%\begin{align}\label{1direst}
%\widehat{V}(f,\{v_j\}_{j=0}^{n})=\sum_{j=1}^{n}\left|f(v_{j+1})-f(v_{j}) - \frac{f(b)-f(a)}{n}\right|.
%\end{align}
%
%For Simpson's rule, we use $\widehat{V}(f''',\{u_j\}_{j=0}^{n+1})$ to approximate $\Var(f''')$, where
%\begin{equation}\label{vhatlessvar}
...
\cc:=\left\{f\in \cv^{p}, \Var(f^{(p)})\leq \mathfrak{C}(\text{size}(\{x_i\}_{i=0}^{n+1}))\widehat{V}(f^{(p)},\{x_i\}_{i=0}^{n+1}),\right.\\ \left.\text{for all choices of } n\in \mathbb{N}, \text{and }\{x_i\}_{i=0}^{n+1} \text{ with }\text{size}(\{x_i\}_{i=0}^{n+1})<\mathfrak{h}\right\},
\end{multline}

talk about size and $\mathfrak{h}$
