...
\begin{lem}\label{lemmaerrorboundtrap}
    ...
    \begin{equation}\label{errortrapcone}
      \overline{\textup{err}}(f,n)\leq \frac{(b-a)^2\mathfrak{C}(2(b-a)/n)\widetilde{V}_1(f,n)}{8n^2}.
    \end{equation}
\end{lem}
...
\begin{lem}\label{lemmaerrorboundsim}
    ...
    \begin{equation}\label{errorsimcone}
      \overline{\textup{err}}(f,n)\leq \frac{(b-a)^4\mathfrak{C}((b-a)/n)\widetilde{V}_3(f,n)}{5832n^4}.
    \end{equation}
\end{lem}
...
Our algorithm is guaranteed to work for those functions lying in the cone-shaped function space. In order to decide whether one input function is in the cone or not, it is important to find out a necessary condition for which $f\in\cc^1$. This condition then can be used as a threshold for the algorithm to rule out all the functions such that $f\notin\cc^1$. For those functions, we will propose a method later in this section. This method tries to make the cone function space a more inclusive new one so that it will contain more functions than before. The details will be provided later. Now, we will find out a necessary condition for which $f\in\cc^1$ first. 

%From \eqref{defvar}, we know that 

%\Var(f^{(p)})\leq \mathfrak{C}(\text{size}(\{x_i\}_{i=0}^{n+1}))\widehat{V}(f^{(p)},\{x_i\}_{i=0}^{n+1})
...
Let $\mathfrak{h}\le (b-a)$. Set $k=1$. Let $\eta_{0}=\infty$, $n_1=\lceil2(b-a)/\mathfrak{h}\rceil$. For any error tolerance $\varepsilon$ and input function $f$, do the following:
\begin{description}
\item[Step 1.\ Estimate and bound {$\Var(f')$}.] Compute  $\widetilde{V}_1(f,n_k)$ in \eqref{vtilde1} and $T_{n_k}(f)$ in \eqref{traprule}.
...
\begin{enumerate}[label=\alph*)]
  \item If $\widetilde{V}_1(f,n_k) \le \eta_{k}$, then go to step 3.
  \item Otherwise, set $\mathfrak{h} = \mathfrak{h}/2$.  If
    \begin{enumerate}[label=(\roman*)]
      \item $n_k \ge 2(b-a)/\mathfrak{h}$, then go to step 3.
      \item Otherwise, choose $$n_{k+1}=n_k\times\left\lceil\frac{1}{2n_k\mathfrak{h}}\right\rceil.$$
        (need double check)
        Go to Step 1.
    \end{enumerate}
\end{enumerate}
...
\item[Step 3. Check for convergence.] Check whether $n_k$ is large enough to satisfy the error tolerance, i.e.
    \begin{equation*}
        \widetilde{V}_1(f,n_k) \le \frac{8\varepsilon n_k^2}{(b-a)^2\mathfrak{C}(2(b-a)/n_k)}.
    \end{equation*}

    \begin{enumerate}[label=\alph*)]
      \item If this is true, then return $T_{n_k}(f)$ and terminate the algorithm.
      \item If this is not true, go the Step 4.
    \end{enumerate}


\item[Step 4. Increase number of inputs and loop again.] Choose
$$
...
\Subsection{Computational Cost of Trapezoidal Rule}
...
\begin{theorem}\label{uppbndcostSimp}
    Let $N(f,\varepsilon)$ denote the final number of $n_k$ in Step 3 when the algorithm terminates. Then this number is bounded below and above in terms of the true, yet unknown, $\Var(f')$.
...
\begin{proof}
  ...
  No matter what inputs $f$ and $\varepsilon$ are provided, $N(f,\varepsilon)\ge n_1=(\lfloor (b-a)/\mathfrak{h}\rfloor+1)$. Then the number of intervals increases until $\widetilde{\text{err}}(f,n)\le\varepsilon$, which by \eqref{errorboundcone} implies that $\overline{\text{err}}(f,n)\le\varepsilon$. This implies the lower bound on $N(f,\varepsilon)$.
  ...
  $$n^*\leq\max\left(\left\lfloor\frac{2(b-a)}{\alpha\mathfrak{h}}\right\rfloor+1,(b-a)\left(\frac{\mathfrak{C}(\alpha\mathfrak{h})\Var(f''')}{8\varepsilon}\right)^{1/4}+1\right),\quad 0<\alpha<1.$$
  ...
  \begin{align*}
    ...
    &=n^*-1\left(\frac{(b-a)^4\mathfrak{C}(2(b-a)/(n^*-1))\Var(f''')}{8(n^*-1)^4\varepsilon}\right)^{1/4},\\
    &\le(b-a)\left(\frac{\mathfrak{C}(\alpha\mathfrak{h})\Var(f''')}{8\varepsilon}\right)^{1/4}.
  \end{align*}
...
Let the sequence of algorithms $\{S_n\}_{n\in \mathcal{I}}$, %$ \widehat{V}(f,\{x_i\}_{i=0}^{n+1})$,
and $\widetilde{V}_3(f,n)$ be as described above.
Let $\mathfrak{h}\le (b-a)/6$. Set $k=1$. Let $\eta_{0}=\infty$, $n_1=\lceil(b-a)/\mathfrak{h}\rceil$. For any error tolerance $\varepsilon$ and input function $f$, do the following:
\begin{description}
\item[Step 1.\ Estimate and bound {$\Var(f''')$}.] Compute  $\widetilde{V}_3(f,n_k)$ in \eqref{vtilde3} and $S_{n_k}(f)$ in \eqref{simrule}.

\item[Step 2. Check the necessary condition for $f \in \cc^3$.] Compute
    \begin{align*}
        \eta_{k}=\min\left(\eta_{k-1},\mathfrak{C}((b-a)/n_{k})\widetilde{V}_3(f,n_k)\right).
    \end{align*}

\begin{enumerate}[label=\alph*)]
  \item If $\widetilde{V}_3(f,n_k) \le \eta_{k}$, then go to step 3.
  \item Otherwise, set $\mathfrak{h} = \mathfrak{h}/2$.  If
    \begin{enumerate}[label=(\roman*)]
      \item $n_k \ge (b-a)/\mathfrak{h}$, then go to step 3.
      \item Otherwise, choose $$n_{k+1}=n_k\times\left\lceil\frac{1}{n_k\mathfrak{h}}\right\rceil.$$
        (need double check)
        Go to Step 1.
    \end{enumerate}
\end{enumerate}
...
\item[Step 3. Check for convergence.] Check whether $n_k$ is large enough to satisfy the error tolerance, i.e.
    \begin{equation*}
        \widetilde{V}_3(f,n_k) \le \frac{5832\varepsilon n_{k}^4}{(b-a)^4\mathfrak{C}((b-a)/n_{k})}.
    \end{equation*}

    \begin{enumerate}[label=\alph*)]
      \item If this is true, then return $S_{n_k}(f)$ and terminate the algorithm.
      \item If this is not true, go the Step 4.
    \end{enumerate}
...
Once we obtained the upper bound of the computational cost, it is important to discover the lower bound of complexity for the univariate integration problems. The computational complexity of a certain problem is the lowest cost required for the best possible algorithm for this problem, including unknown algorithms. If the upper bound of the computational cost of our algorithm is to the same order asymptotically. It is confident to say that our algorithm has the optimal cost for solving the problem. In the next chapter, we will discuss the complexity of the problem.
...
Firstly, we derive a lower bound on the cost of approximating functions in the ball $\cb_{\sigma}$ and in the cone $\cc_{\tau}$ by constructing fooling functions. We choose  the triangle shaped function $f_0: x \mapsto 1/2-\abs{1/2-x}$. Then
\begin{gather*}
...
\begin{theorem}\label{lowbndcost}
    ...
    \end{equation}
    function values. As $\Var(f''')/\varepsilon \rightarrow \infty$ the asymptotic rate of increase is the same as the computational cost of \textup{integral}.
\end{theorem}
\begin{proof}
